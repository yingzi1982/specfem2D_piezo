...
!PIEZO
  ...
  use piezo_par, only : is_piezo, voltage_time_function
!PIEZO
...
!PIEZO
  real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLZ) :: sigma_xx_piezo,sigma_zz_piezo,sigma_xz_piezo,sigma_zx_piezo
  real(kind=CUSTOM_REAL) :: dummy_sigma_xx_piezo,dummy_sigma_zz_piezo,dummy_sigma_xz_piezo,dummy_sigma_zx_piezo
  real(kind=CUSTOM_REAL) :: dummy_coord_x, dummy_coord_z
!PIEZO
...
!PIEZO
    !if ( (is_piezo) .and. (it==1) ) then
    if ( (is_piezo) ) then
      do j = 1,NGLLZ
	do i = 1,NGLLX
          iglob = ibool(i,j,ispec)
          dummy_coord_x = coord(1,iglob)
          dummy_coord_z = coord(2,iglob)
          call calculate_piezo_stress(dummy_coord_x,dummy_coord_z,dummy_sigma_xx_piezo,dummy_sigma_zz_piezo,dummy_sigma_xz_piezo,dummy_sigma_zx_piezo)
          sigma_xx_piezo(i,j) = dummy_sigma_xx_piezo
	  sigma_zz_piezo(i,j) = dummy_sigma_zz_piezo
          sigma_xz_piezo(i,j) = dummy_sigma_xz_piezo
          sigma_zx_piezo(i,j) = dummy_sigma_zx_piezo
        enddo
      enddo
      !write(IMAIN,*) 'rank = ', myrank, 'it = ', it, 'max sigma_piezo = ', maxval(sigma_xx_piezo), maxval(sigma_zz_piezo), maxval(sigma_xz_piezo)
    endif
!PIEZO
...
!PIEZO
        !if ((is_piezo) .and. (.not.(ispec_is_PML(ispec)))) then
        if ((is_piezo)) then
          sigma_xx = sigma_xx + sigma_xx_piezo(i,j) * voltage_time_function(it)
          sigma_zz = sigma_zz + sigma_zz_piezo(i,j) * voltage_time_function(it)
          sigma_xz = sigma_xz + sigma_xz_piezo(i,j) * voltage_time_function(it)
          sigma_zx = sigma_zx + sigma_zx_piezo(i,j) * voltage_time_function(it)
        endif
!PIEZO
