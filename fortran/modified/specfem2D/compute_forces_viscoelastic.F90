...
!PIEZO
  ...
  real(kind=CUSTOM_REAL), dimension(3) :: dummy_sigma_piezo
...
!PIEZO
    dummy_coord(:) = 0._CUSTOM_REAL
    dummy_sigma_piezo(:) = 0._CUSTOM_REAL

    sigma_xx_piezo(:,:) = 0._CUSTOM_REAL 
    sigma_zz_piezo(:,:) = 0._CUSTOM_REAL
    sigma_xz_piezo(:,:) = 0._CUSTOM_REAL
    sigma_zx_piezo(:,:) = 0._CUSTOM_REAL

    if ((is_piezo)) then !.and. (it==1)
      !do j = 1,NGLLZ
	!do i = 1,NGLLX
      do j = 1,1
	do i = 1,1
          iglob = ibool(i,j,ispec)
          dummy_coord = [coord(1,iglob),coord(2,iglob)]
          !dummy_coord = coord(1:2,iglob)
          call calculate_piezo_stress(dummy_coord,dummy_sigma_piezo)
          !write(IMAIN,*) i,j,dummy_coord(1), dummy_coord(2), dummy_sigma_piezo(1), dummy_sigma_piezo(2), dummy_sigma_piezo(3)
          sigma_xx_piezo(i,j) = dummy_sigma_piezo(1)
	  sigma_zz_piezo(i,j) = dummy_sigma_piezo(2)
          sigma_xz_piezo(i,j) = dummy_sigma_piezo(3)
          sigma_zx_piezo(i,j) = dummy_sigma_piezo(3)
        enddo
      enddo
    endif
!PIEZO
